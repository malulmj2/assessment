...
public class DiffEngineServiceTest {
	...
	assertCalculate(DiffAction.CREATED, diff.getRecord().getAction(), "A null original object diffed against non-null modified object must reflect as being “created” in diff.",
		original, modified, diff.getRecord().getOriginalValue(), diff.getRecord().getModifiedValue());
	assertFalse("There should be diff records as original is not the same as modified.", diff.getRecord().getRecords().isEmpty());

	for (DiffRecord record : diff.getRecord().getRecords()) {
	    if (!"serialVersionUID".equals(record.getFieldName())) {
		...
		assertEquals("A null original object diffed against non-null modified object must reflect as being “created” in diff.", DiffAction.CREATED, record.getAction());
	    }
	...
	Diff<Person> diff = diffEngine.calculate(original, modified);
	assertCalculate(DiffAction.DELETED, diff.getRecord().getAction(), "A non-null original object diffed against null modified object must reflect as being “deleted” in diff",
		original, modified, diff.getRecord().getOriginalValue(), diff.getRecord().getModifiedValue());
	assertTrue("There should be no diff records as the whole object was deleted", diff.getRecord().getRecords().isEmpty());
	...
	assertCalculate(DiffAction.UPDATED, diff.getRecord().getAction(),
		...
		original, modified, diff.getRecord().getOriginalValue(), diff.getRecord().getModifiedValue());
	assertTrue("Only surname and friend should be updated as they have been modified", diff.getRecord().getRecords().size() == 2);
	...
	assertEquals("The result returned from diffService.apply(original,diff) must be equal to modified", modified, actual);
    }
	...
	assertTrue("Only firstName and friend should be updated as they have been modified", diff.getRecord().getRecords().size() == 2);
	...
	for (DiffRecord record : diff.getRecord().getRecords()) {
		...
		assertEquals(
			"The Person object fro freind should be updated",
			"Person", record.getRecords().get(0).getFieldName());
		for (DiffRecord diffRecord : record.getRecords().get(0).getRecords()) {
		    switch (diffRecord.getFieldName()) {
		    case "firstName":
			assertEquals("The new value for FirstName should be \"Jim\"", "Jim", diffRecord.getModifiedValue());
			break;
		    case "surname":
	...
	for (DiffRecord record : diff.getRecord().getRecords()) {
		...
		assertEquals(
			...
			DiffAction.UPDATED, record.getAction());
		assertEquals(
			"The friend should be deleted",
			DiffAction.DELETED,  record.getRecords().get(0).getAction());
	...
	for (DiffRecord diffRecord : record.getRecords()) {
	    ...
	    assertEquals("A null original object diffed against non-null modified object must reflect as being “created” in diff.", DiffAction.CREATED, diffRecord.getAction());
	}
    }

    private void assertCalculate(DiffAction expectedAction, DiffAction actualAction, String actionMessage, Object expectedOriginalValue, Object expectedNewValue,
	    Object actualOriginalValue, Object ectualNewValue) {
	assertEquals(actionMessage, expectedAction, actualAction);
	assertEquals("The orginal value should be as it was before modification", expectedOriginalValue, actualOriginalValue);
